...
//GET ALL CANDY AND THEIR PRODUCER
router.route('/')
    .get((req, res, next) => {
        ...
        pool((err, connection) => {
            connection.query(
                ...
                result.forEach(c => {
                    ...
                    output += `<p>${c.Candy} - ${c.Category} - ${c.Producer} - ${c.Price} <button onclick="buyCandy(${c.ID})">Köp</button><script>
                    function buyCandy(id) {
                        console.log("Godiset med id " + id + " tillagt i ägget")
                    }
                </script>`
                });
                res.send(output)
...
//GET ONE CANDY AND ITS PRODUCER
router.route('/:id')
    .get((req, res, next) => {
        pool((err, connection) => {
            connection.query(
            `SELECT candy.name AS "Candy", candy.category AS "Category", candy.color AS "Color", producers.name as "Producer", candy_producers.price_per_unit AS "Price/unit" 
            FROM candy
            LEFT JOIN candy_producers ON candy.id = candy_producers.candy_id
            LEFT JOIN producers on producers.id = candy_producers.producer_id
            WHERE candy.id= ${connection.escape(req.params.id)}`, (error, result, fields) => {
                connection.release()
                if (error) throw error
                res.json(result)
            })
